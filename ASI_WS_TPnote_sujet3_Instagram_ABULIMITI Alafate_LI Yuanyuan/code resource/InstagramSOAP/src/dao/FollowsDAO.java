...
public class FollowsDAO 
{
	...
	{
		try {
			if(getFollow(follow.getFollow().getuId(), follow.getFollower().getuId())==null || getFollow(follow.getFollow().getuId(), follow.getFollower().getuId()).size()==0)
			{
				Session session = HibernateUtil.currentSession();
	...
	/**
	 * convert a 'Follows' to a string with id and name.
	 * @param follows : 'Follows'
	 * @return string
	 */
	public String convertToAString(Follows follows){
		String result="follow id: "+follows.getFollow().getuId()+", "+
				"follow name: "+follows.getFollow().getUserName()+", "+
				"follower id: "+follows.getFollower().getuId()+", "+
				"follower name: "+follows.getFollower().getUserName();
		return result;
	}
}

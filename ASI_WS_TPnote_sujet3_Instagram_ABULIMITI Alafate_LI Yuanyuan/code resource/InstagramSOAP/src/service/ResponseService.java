...
public class ResponseService {
	...
	public String convertToAString(Response response) {
		return responseDAO.convertToAString(response);
	}

	/**
	 * convert a list of response to a list of string
	 * 
	 * @param responses
	 *            : a list of response
	 * @return a list of string
	 */
	public List<String> convertToListString(List<Response> responses) {
		List<String> responseLists = new ArrayList<>();
		if (responses != null) {
			for (int i = 0; i < responses.size(); i++) {
				responseLists.add(convertToAString(responses.get(i)));
			}
		}
		return responseLists;
	}
